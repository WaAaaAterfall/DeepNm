...
def train_diff_model(config):
    ...
    pidx = train_out == 1
    nidx = np.where(train_out == 0)[0]
    nidx = nidx[np.random.permutation(len(nidx))[:sum(pidx)*1]]
    train_seq = np.concatenate([train_seq[pidx], train_seq[nidx]])
    train_out = np.concatenate([np.ones(sum(pidx)), np.zeros(len(nidx))])
    ...
    train_seq = train_seq[:, 425:576, :].astype(np.float32)
    valid_seq = valid_seq[:, 425:576, :].astype(np.float32)
    ...
    if c.nano:
        ...
        train_nano = np.concatenate([train_nano[pidx], train_nano[nidx]])
