...
class Seq2pO3(tf.keras.Model):
    def __init__(self):
        super(Seq2pO3, self).__init__()

        self.conv1 = tfkl.Conv1D(32, 3, padding='same', activation='relu',
                                 kernel_regularizer=l2(0.001))
        self.dropout = tfkl.Dropout(0.6)
        # self.rnn1 = tfkl.Bidirectional(tfkl.LSTM(
        #     16, dropout=0.2, kernel_regularizer=l2(0.001), return_sequences=False))

        self.fc = tfkl.Dense(1, activation='sigmoid')
    def call(self, inputs, training=True, mask=None):
        seq = inputs
        h1 = self.conv1(seq)
        h1 = self.dropout(h1)
